...
var   logFile = fso.CreateTextFile("ConvertMatched.log", true);
var   convertSize = 10;
var   i, j;
var   TO_CONVERT = "Matched AAC audio file";

function getAllTracksToConvert(tracksCollection)
{
//	var   matchedFile = fso.CreateTextFile("Matched.txt", true);
	var   k;
	var   matchedTracks = [];
	var   matchedCount = 0;
	...
	for (k = 1; k <= tracksCollection.Count; k++) {
		     ...
		     ( fso.FileExists( currTrack.Location ) ) )
		{
			...
			matchedCount++;
			matchedTracks.push( currTrack );
		}
	}
	logFile.WriteLine("Found " + matchedCount + " track(s) of kind " + TO_CONVERT + " to convert. " );
	logFile.Close();
